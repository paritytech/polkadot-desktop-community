 ...
 */
export const sendChatRequestV2 = async (params: {
  ...
  const allPeerTopic = computeAllPeerTopic(recipientAccountId);
  const paginationTopic = computePaginationTopic(recipientAccountId, currentDay.day);
  ...
  const sharedSecret = computeSharedSecret(ephemeralPrivKey, recipientChatPubKey);
  const encryption = createEncryption(sharedSecret);

  // 3. Channel topic for accept-signal monitoring.
  const channelTopic = computeChannelTopic(ephemeralPubKey, sharedSecret);
  ...
  const identityProofSharedSecret = computeSharedSecret(senderIdentityChatPrivateKey, recipientChatPubKey);
  ...
  const remoteModelEncoded = RemoteModel.enc(remoteModel);
 ...
 */
export const decryptAndValidateRequestV2 = (
  ...
  try {
    ...
    const sharedSecret = computeSharedSecret(ownChatP256PrivateKey, encrypted.encryptionPubKey);
    const encryption = createEncryption(sharedSecret);
    ...
    const channelTopic = toHex(computeChannelTopic(encrypted.encryptionPubKey, sharedSecret));
 ...
 */
export const subscribeToIncomingRequestsV2 = (
  params: {
  ...
  const paginationTopic = computePaginationTopic(ownAccountId, currentDay.day);
