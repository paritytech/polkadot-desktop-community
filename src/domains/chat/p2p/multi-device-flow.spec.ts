...
} from './multi-device-flow';
import { type DeviceTarget } from './multi-device-request';
...
describe('encryptForRecipients / decryptForOwnDevice (request side)', () => {
  it('round-trips a payload to a single recipient device', () => {
    ...
    const { data } = encryptForRecipients(plaintext, [recipient.target], sender.privateKey);
    const decrypted = decryptForOwnDevice(data, recipient.target.statementAccountId, recipient.privateKey, sender.publicKey);
  ...
  it('delivers the same plaintext to every recipient device', () => {
    ...
    const { data } = encryptForRecipients(plaintext, [r1.target, r2.target, r3.target], sender.privateKey);

    expect(decryptForOwnDevice(data, r1.target.statementAccountId, r1.privateKey, sender.publicKey)).toEqual(plaintext);
    expect(decryptForOwnDevice(data, r2.target.statementAccountId, r2.privateKey, sender.publicKey)).toEqual(plaintext);
    expect(decryptForOwnDevice(data, r3.target.statementAccountId, r3.privateKey, sender.publicKey)).toEqual(plaintext);
  });
  ...
  it('returns null when the receiver is not in the recipient list', () => {
    ...
    expect(decryptForOwnDevice(data, stranger.target.statementAccountId, stranger.privateKey, sender.publicKey)).toBeNull();
  });
  ...
  it('returns null when the recipient uses the wrong encryption private key', () => {
    ...
    expect(decryptForOwnDevice(data, recipient.target.statementAccountId, wrongPriv, sender.publicKey)).toBeNull();
  });
  ...
  it('returns null on a corrupted ciphertext', () => {
    ...
    expect(decryptForOwnDevice(tampered, recipient.target.statementAccountId, recipient.privateKey, sender.publicKey)).toBeNull();
  });
  ...
  it('returns null on a non-MultiDeviceRequest byte sequence (graceful decode failure)', () => {
    ...
    expect(decryptForOwnDevice(garbage, recipient.target.statementAccountId, recipient.privateKey, sender.publicKey)).toBeNull();
  });
  ...
  it('produces a different ciphertext on each encrypt (random nonce + one-shot key)', () => {
    ...
    const a = encryptForRecipients(plaintext, [recipient.target], sender.privateKey);
    const b = encryptForRecipients(plaintext, [recipient.target], sender.privateKey);
  ...
  it('returns null when the sender pubkey does not match the actual encrypter', () => {
    ...
    expect(
      decryptForOwnDevice(data, recipient.target.statementAccountId, recipient.privateKey, otherSender.publicKey),
    ).toBeNull();
  });
});

describe('encryptResponseForRecipients / decryptResponseForOwnDevice', () => {
  it('round-trips a response payload', () => {
    const sender = generateP256KeyPair();
    const recipient = makeRecipient(0xb1);
    const plaintext = new TextEncoder().encode('response body');

    const { data } = encryptResponseForRecipients(plaintext, [recipient.target], sender.privateKey);
    const decrypted = decryptResponseForOwnDevice(
      data,
