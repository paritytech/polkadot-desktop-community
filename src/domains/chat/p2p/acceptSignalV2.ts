/**
 * V2 identity-level transport — publish/subscribe on `SessionId(A,B)` keyed by
 * `K(A,B) = ECDH(identityChatPriv, peerIdentityChatPub)`.
 *
 * Spec v0.1 routes a few
 * chat-content variants through this identity-level channel rather than the
 * per-device session topic:
 *
...
import { MultiChatAccepted, SingleRequest, StructuredStatementData } from './chatRequestCodec';
import { computeSharedSecret } from './keys';
import { decryptForOwnDevice } from './multi-device-flow';
...
const computeRoute = (
  ...
  const sharedSecret = computeSharedSecret(keys.ownIdentityChatPrivateKey, keys.peerIdentityChatPublicKey);
...
/**
 * Per-peer-device "main" channel — matches `chatSessionV2.ts:135` outgoing
...
const computeDeviceRoute = (
  ...
  const sharedSecret =
    direction === 'outgoing'
      ? computeSharedSecret(keys.ownDeviceEncryptionPrivateKey, keys.peerIdentityChatPublicKey)
      : computeSharedSecret(keys.ownIdentityChatPrivateKey, keys.peerDeviceEncryptionPublicKey);
 ...
 */
export const postChatMessageOnIdentityChannel = async (
 ...
 */
export const postChatMessageOnDeviceChannel = async (
  params: DeviceRoute & {
    signerDeviceSeed: Uint8Array;
    statementStore: StatementStoreAdapter;
    chatMessageContent: CodecType<typeof ChatMessageCodec>['versioned']['value'];
...
export const encodeMultiChatAccepted = (params: {
  requestId: string;
  acceptorDevices: { statementAccountId: Uint8Array; encryptionPublicKey: Uint8Array }[];
}): Uint8Array => MultiChatAccepted.enc(params);

export const postAcceptSignalV2 = async (
  params: IdentityRoute & {
    signerDeviceSeed: Uint8Array;
    payload: Uint8Array;
    statementStore: StatementStoreAdapter;
...
export const decodeEventsFromChatMessage = (msgBytes: Uint8Array): IdentityChannelEvent[] => {
...
const decodeIdentityChannelEvents = (
  ...
  if (outer.tag === 'Request') {
    ...
    const unwrapped = decryptForOwnDevice(
      outerPlaintext,
 ...
 */
export const subscribeToIdentityChannelV2 = (
  ...
  return trackedSubscribeStatements(statementStore, { matchAll: [topic] }, ({ statements }) => {
 ...
 */
export const subscribeToAcceptSignalV2 = (
  params: IdentityRoute & { statementStore: StatementStoreAdapter },
