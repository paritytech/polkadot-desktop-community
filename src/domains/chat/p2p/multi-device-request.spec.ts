...
import { MultiDeviceRequest, MultiDeviceResponse } from './chatRequestCodec';
import {
  type DeviceTarget,
  type KeyWrap,
  type SymmetricEncrypt,
  buildMultiDeviceRequest,
  buildMultiDeviceResponse,
} from './multi-device-request';
...
describe('buildMultiDeviceRequest', () => {
  ...
  it('preserves the recipient statementAccountId on each devicesInfo entry', () => {
    ...
    const built = buildMultiDeviceRequest({
...
describe('buildMultiDeviceResponse', () => {
  it('mirrors the request builder under the response field name', () => {
    const built = buildMultiDeviceResponse({
