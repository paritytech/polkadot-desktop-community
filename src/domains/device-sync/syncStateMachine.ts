...
export function startSyncStateMachine(params: SyncStateMachineParams): SyncStateMachineHandle {
  ...
  console.info('WEBRTC [sync] started peer=%s dataChannel.state=%s', params.peerStatementAccountId, dataChannel.readyState);
  ...
  async function onMessage(ev: MessageEvent<ArrayBuffer | Uint8Array>): Promise<void> {
    ...
    console.info('WEBRTC [sync] IN envelope id=%s dataLen=%d', envelope.id, envelope.data.length);
    if (envelope.id !== DEVICE_SYNC_USE_CASE_ID) {
      console.info('WEBRTC [sync] IN envelope id=%s ignored (not device-sync use-case)', envelope.id);
      return;
    }
